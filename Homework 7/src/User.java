...
public class User implements Comparable<User>{
    ...
    public ArrayList<Meme> getMemesViewed() {
        return new ArrayList<>(memesViewed);
    }
    ...
    public void setMemesViewed(ArrayList<Meme> memesViewed) {
        this.memesViewed.addAll(memesViewed);
    }

    /**
     * Rates any given meme with the rating parameter. Calls on the java.Meme's addRating method during processing
     ...
     * @return Returns the new java.Meme
     */
     ...
     * Note: This is an unstable method, any user can set any meme to be shared, regardless of who owns it (if they have access to the java.Meme itself)
     *
     ...
     *
     * @param feed java.Feed to get a new java.Meme from
...
//        return String.format("%s has rated %d memes, %d", userName, memesViewed.size(), calculateReputation());
    ...
    public int compareTo(User user) {
        if ( user.getUserName().equals(userName) ) {
            return user.getMemesCreated().size() - memesCreated.size();
        }
